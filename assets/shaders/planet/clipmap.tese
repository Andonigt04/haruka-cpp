...
layout(std140, binding = 13) uniform ClipParams {
    vec4 uClipOrigin; vec4 uClipTanU; vec4 uClipTanV;
};
...
void main() {
    ...
    //   meshM  lo que usa el eval de la MALLA a esta misma distancia.
    //
    ...
    float meshM = max(length(dir * baseR + uCenter.xyz) * 0.006, 2.0);
    float triM  = mix(clipM, meshM, smoothstep(1400.0, 1900.0, rad));
    float h     = harukaTerrainDetail(dir, baseR, triM) * att;
    ...
    const float eps = 3.0;                        // más fino que en la malla: aquí el paso es de 2 m
