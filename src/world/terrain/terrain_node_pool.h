...
public:
    ...
    /** @brief Recalcula el rango de TODOS los residentes con `f(node)`. Para cuando cambia la
     *  fuente de la altura (`setHeightSampler`): el pool es una cache y los nodos que ya estaban
     *  guardan el rango de la fuente ANTERIOR — o ninguno, si no habia. Medido en el banco: con un
     *  muestreador nuevo, 1 nodo acotado bajo el agua de los ~180 en pantalla, porque solo el
     *  recien generado sabia su fondo; el resto heredaba un rango invalido de raices generadas
     *  sin muestreador. */
    template <class F> void rerange(F&& f) {
        for (Slot& s : m_slots) if (s.used) s.range = f(s.node);
    }
