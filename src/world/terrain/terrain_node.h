...
#include "world/terrain/cube_sphere.h"             // cubeFaceToDir — la proyección, que NO se reimplementa aquí
#include "world/terrain/terrain_detail.h"   // terrainDetail(dvec3,...) — la altura, gemela del .glsl
...
/**
 * @brief Tope DURO de nivel: lo que cabe en la clave y en los índices. NO es el nivel al que se
 *        subdivide — ése lo da `nodeMaxLevelFor(R)`, que depende del planeta.
 *
 * La clave del pool reparte 64 bits en 3 de cara, 6 de nivel y 2×26 de índice (`nodeKey`), así que
 * `i, j < 2^26` y el nivel no puede pasar de 26. Es un límite de REPRESENTACIÓN: si un planeta pide
 * más (`nodeMaxLevelFor` lo recorta aquí), el téxel más fino se queda más basto que la octava más
 * fina y `terrain_node_scale` lo dice. Con λ 4,5 m eso empieza en R ≈ 965 R⊕ (nueve soles): no es un caso real.
 */
inline constexpr uint32_t TERRAIN_NODE_MAX_LEVEL = 26u;

/// Longitud de onda de la octava MÁS FINA de `terrain_detail.h` (m). Gemela de la tabla de allí y de
/// la de `nodeMissingReliefM`: es lo que fija hasta dónde tiene sentido subdividir.
inline constexpr double TERRAIN_NODE_FINEST_LAMBDA_M = 4.5;

/**
 * @brief Nivel máximo al que se subdivide un planeta de radio `R`. **Lo fija la escalera de ruido,
 *        NO la pantalla, y depende del radio**: a más planeta, más niveles para el mismo téxel.
 *
 * ⚠️ ERA LA CONSTANTE 17, CALCULADA PARA LA TIERRA. Estaba bien para R = 6 371 km (0,60 m/téxel) y
 * mal para cualquier otro: un planeta de 4 R⊕ se quedaba en 2,4 m/téxel y perdía la octava de 4,5 m
 * entera; uno de R⊕/4 gastaba dos niveles en interpolarse a sí mismo. Andoni: "cuanto más grande el
 * planeta más definición, pero automático".
 *
 * La regla es la misma que justificaba el 17: la octava más fina entra a peso completo cuando el
 * corte baja de λ/4 (`octaveWeight` = clamp(λ/(2m) − 1)), o sea 1,125 m. El primer nivel cuyo téxel
 * (`nodeTexelM`: R·π/2 / 2^L / 128) baja de ahí es el tope. Para la Tierra sale 17, como antes.
 * Por debajo de λ/4 el nodo guarda una interpolación de sí mismo: información que no existe, y que
 * se paga — medido a 2 m de altura, los niveles 18-20 eran **2 732 de 5 439 nodos** cuando el tope
 * era 20. El detalle más fino que eso es cosa de las TEXTURAS del material, no de la geometría.
 */
inline uint32_t nodeMaxLevelFor(double planetRadiusM) {
    const double rootTexelM = planetRadiusM * 1.5707963267948966 / (double)TERRAIN_NODE_CELLS;
    const double wantM      = TERRAIN_NODE_FINEST_LAMBDA_M * 0.25;
    const double L = std::ceil(std::log2(std::max(rootTexelM / wantM, 1.0)));
    return (uint32_t)std::min(L, (double)TERRAIN_NODE_MAX_LEVEL);
}
...
/**
 * @brief LA RAMPA DEL MAPA POR ZANCADA: el corte de octavas que se dibuja pasa de un mapa al
 *        siguiente de forma CONTINUA en la distancia, en vez de saltar en el radio de la zancada.
 *        Fracción del radio que ocupa la rampa antes de cada umbral (ver `nodeStrideMapF`).
 *
 * ── LOS CÍRCULOS EN EL TERRENO (Andoni, 19-09: "sigue ocurriendo y no es de agua sino de terreno") ─
 *
 * Con zancada `s` el vértice lee el mapa `log2(s)` niveles arriba (`skMap` en `terrain_node.vert`),
 * o sea el relieve cortado a `téxel × s`. La zancada es 1 hasta `TERRAIN_NODE_STRIDE_MATCH_M`, 2
 * hasta el doble y 4 después (con `vertexPx/errorPx = 4`), así que a 150 y a 300 m del observador
 * el relieve dibujado SUELTA UNA OCTAVA de golpe: 0,38 m de media (quad 4,8 m) y hasta 3,75 m el
 * peor (`nodeMissingReliefM`). En el sombreado eso es un cambio de RUGOSIDAD en una circunferencia
 * alrededor de la cámara — los círculos. La vista de depuración 4 los pinta por construcción.
 *
 * ⚠️ NO ES UN CORTE NUEVO: ES EL MISMO ESCALÓN, REPARTIDO. En la banda `[D·(1−ramp), D]` antes de
 * cada radio `D = STRIDE_MATCH · 2^k` el vértice mezcla el mapa `k` con el `k+1` por su propia
 * distancia (misma fórmula que el morph por distancia: función de (nivel, posición, cámara), así
 * que las dos caras de una arista compartida dan lo mismo). A `D·(1−ramp)` se dibuja lo de siempre
 * y a `D` lo que se dibujaba un metro más allá; en medio, una mezcla lineal de los dos. La
 * superficie dibujada queda entre las dos que ya se dibujaban a un lado y otro del radio: no
 * aparece disparidad con lo que se pisa que no existiera ya (Andoni: "no quiero añadir disparidad
 * en ningún concepto"). Lo que se pierde: en `[D·(1−ramp), D]` el relieve se acerca al corte grueso
 * unos metros antes que antes.
 *
 * ⚠️ EL MAPA NUNCA ES MÁS FINO QUE LA ZANCADA DEL NODO (`fEff = max(fMap, skMap)`): una malla a
 * zancada 2 leyendo el corte del téxel es sub-Nyquist y hace picos (medido en
 * `terrain_node_octave_cut_nyquist`: 4,5× de laplaciano). Como la zancada se decide por el punto
 * MÁS CERCANO del nodo y la rampa es monótona en la distancia, todos los vértices del nodo tienen
 * `fMap >= log2(zancada)` salvo por la histéresis (±25 % de `want`), donde el `max` mete un
 * escalón de como mucho 0,32 octava en la frontera del nodo.
 *
 * El agua hace LO MISMO con su corte analítico (`terrain_node_water.vert`: mezcla de dos
 * `harukaTerrainDetail`), o la lámina se apoyaría en un fondo distinto del que se dibuja.
 *
 * 0 = sin rampa (el escalón de antes); es la contraprueba del banco (`setStrideRampOverride`).
 */
inline constexpr double TERRAIN_NODE_STRIDE_RAMP = 0.25;

/**
 * @brief Índice FRACCIONARIO del mapa que lee un vértice a `distM` del observador. Gemelo del
 *        bloque de `fMap` en `terrain_node.vert` y `terrain_node_water.vert`.
 *
 * Dos tramos, que empalman en `2·strideMatchM` (300 m):
 *
 * · CERCA: `log2` de la MISMA ley que `nodeStrideWant` (`min(screenWant, collWant, tope de cerca)`),
 *   evaluada en el vértice y con el tope de cerca hecho rampa en vez de escalón: 1 hasta
 *   `D·(1−ramp)`, subiendo a 2 en `D = strideMatchM`, y lo mismo hacia 4 en `2D`. Fuera de las rampas
 *   coincide con la zancada del nodo (cuantizada hacia abajo): bajo los pies no cambia nada.
 *
 * · LEJOS (`radPerPx > 0`): **EL CORTE CONTINUO** — el espaciado del mapa leído mide `vertexPx`
 *   píxeles a cualquier distancia: `f = log2(vertexPx / téxel_px)`, con `téxel_px = téxel /
 *   (dist · radPerPx)`. Dentro de una banda de nivel el téxel va de `errorPx` a `errorPx/2` píxeles,
 *   así que `f` recorre exactamente una octava (2 → 3 con `vertexPx/errorPx = 4`) y en la frontera
 *   el hijo lee `8·téxel` = el `4·téxel` del padre: **continuo por construcción**. Sin esto cada
 *   frontera de nivel era un diente de sierra del corte (16 → 8 px y vuelta a 16): los anillos del
 *   terreno a 614 m, 1,2 km, 2,5 km, 4,9 km (0,38 m de la octava de 22 m a 1,2 km). Necesita el mapa
 *   del BISABUELO (k = 3) al final de cada banda, y el del TATARABUELO (k = 4) para los vértices
 *   lejanos de una hoja a punto de fundirse: el nivel se decide por el punto más cercano del nodo y
 *   `f` va por vértice, así que en la esquina lejana (un 35 % más lejos a 1080p) `f` llega a 3,43.
 *   Recortado en 3 quedaba un escalón de 0,075 m de media en las fronteras (medido). Y hasta 6
 *   (`slot.w` lleva los ancestros 5 y 6) porque la partición bajo agua somera baja nodos dos niveles
 *   más de lo que la pantalla pide: recortados en 4 dibujaban un mapa más fino que el vecino y
 *   abrían una costura recta de una octava en la costa (Andoni, 20-09).
 *   La malla nunca queda sub-Nyquist: su zancada es
 *   `2^floor(log2 want)` ≤ `2^f`. Y `vertexPx` es EL mando: 8 = el lado grueso del diente de hoy en
 *   todas partes, gratis; 4 = el lado fino, a ×3,5 triángulos (VERTPX=4, 22 ms medidos).
 *
 * ⚠️ Lejos SÍ se dibuja más basto que antes en la mitad lejana de cada banda (una octava menos que
 * el diente de sierra dejaba allí): a 1,2 km hasta 0,38 m (0,3 px), a 4,9 km 3,8 m (0,8 px). Es la
 * opción (A) de TODO.md, elegida por Andoni para probar primero; con `vertexPx = 4` es la (B).
 */
inline double nodeStrideMapF(double distM, double texelM, double strideMatchM, double ramp,
                             double screenWant, double fineCellM,
                             double radPerPx = 0.0, double errorPx = TERRAIN_NODE_ERROR_PX) {
    const double collWant = std::max(fineCellM, distM / (double)(TERRAIN_NODE_CELLS / 2))
                          / std::max(texelM, 1e-9);
    double clampC = 4.0;
    if (ramp <= 0.0) clampC = (distM <= strideMatchM) ? 1.0 : (distM <= 2.0 * strideMatchM) ? 2.0 : 4.0;
    else {
        double f = 0.0;
        for (int k = 0; k < 2; ++k) {
            const double D = strideMatchM * (double)(1 << k);
            f += std::min(std::max((distM - D * (1.0 - ramp)) / (D * ramp), 0.0), 1.0);
        }
        clampC = std::exp2(f);
    }
    const double want = std::min(std::min(screenWant, collWant), clampC);
    double f = std::min(std::max(std::log2(std::max(want, 1.0)), 0.0), 2.0);
    if (radPerPx > 0.0 && distM > 2.0 * strideMatchM) {
        const double texelPx = texelM / std::max(distM * radPerPx, 1e-12);
        const double fPx = std::log2(std::max(screenWant * errorPx / std::max(texelPx, 1e-9), 1.0));
        f = std::max(f, std::min(fPx, 6.0));
    }
    return f;
}
...
inline double nodeMissingReliefM(double texelM, double spanM) {
    ...
    static constexpr double kOct[7][2] = {
        ...
        {   625.0,  70.0 }, {  111.0,  14.0 }, {   22.0,   3.0 }, { TERRAIN_NODE_FINEST_LAMBDA_M, 0.7 },
    };
...
/**
 * @brief ¿Este nodo tiene el fondo bajo agua SOMERA y su téxel le esconde más relieve del que la
 *        profundidad tolera? Si sí, hay que partirlo aunque la pantalla no lo pida.
 *
 * Es la misma regla que acota la zancada bajo el agua en `TerrainNodeRenderer` (`kWaterReliefFrac`),
 * aplicada al NIVEL: allí se limita lo que pierde el diezmado respecto al téxel; aquí lo que pierde
 * el téxel respecto al campo entero (`nodeMissingReliefM(0, téxel)`). Las dos se suman.
 *
 * Por qué hace falta: al cruzar la frontera entre dos niveles el téxel se DOBLA y `terrainDetail`
 * suelta una octava. En tierra no se ve. Bajo el agua sí, porque todo lo que la dibuja depende de
 * la profundidad y es no lineal cerca de cero (Green, rompiente, espuma, absorción) — y como el
 * nivel se elige por distancia, la frontera es una circunferencia alrededor de la cámara: los
 * "círculos en la costa". Acotar la zancada quitó las bandas de zancada; las de nivel seguían.
 *
 * ⚠️ NO toca el corte del detalle. Partir más ACERCA lo dibujado al campo que se pisa; lo que no se
 * quiere es lo contrario (Andoni: "no quiero añadir disparidad en ningún concepto").
 *
 * `depth` se mide desde el punto MÁS HONDO del nodo: un nodo de mar abierto (minM = −2 000 m) tiene
 * un presupuesto de 500 m y esto no le pide nada; uno de costa (minM = −3 m) tiene 0,75 m y baja
 * hasta el nivel cuyo téxel esconde menos que eso (en la Tierra, el 14: 4,8 m/téxel, 0,35 m
 * perdidos). Un nodo grande que contiene playa Y abismo se juzga por el abismo — la regla actúa
 * cuando el nodo ya es lo bastante pequeño para ser todo somero, que es donde se ven los círculos.
 */
/// Alcance por defecto de la particion extra bajo agua somera, en multiplos de la distancia de
/// particion normal del nodo (ver `nodeSelectVisible`). Fija el coste, que crece con el CUADRADO
/// del alcance (es un area de nodos finos alrededor de la camara). Medido en
/// `terrain_node_shallow_split` sobre una plataforma plana de 3 m y 30 km (el peor caso):
/// alcance 2 = +80 % de nodos (494 -> 890), 3 = +172 %, 4 = +251 %. Con 2 la unica frontera que
/// pasaba del presupuesto (14->13 a 6 km, 0,42 x profundidad) se va a 12 km y baja a 0,08.
inline constexpr double TERRAIN_NODE_WATER_SPLIT_REACH = 2.0;

inline bool nodeShallowWantsSplit(const NodeId& n, double planetRadiusM, double nodeMinM,
                                  double waterLevelM, double reliefFrac) {
    if (reliefFrac <= 0.0 || !(nodeMinM < waterLevelM)) return false;
    const double depth  = std::max(waterLevelM - nodeMinM, 1.0);
    const double texelM = nodeTexelM(n, planetRadiusM);
    return nodeMissingReliefM(0.0, texelM) > reliefFrac * depth;
}
...
inline bool nodeShouldSplit(const NodeId& n, double planetRadiusM,
    ...
    if (n.level >= nodeMaxLevelFor(planetRadiusM)) return false;
 ...
 * el proceso.
 *
 * `waterLevelM`, `waterReliefFrac`, `waterReach`: la partición EXTRA bajo agua somera, ver
 * `nodeShallowWantsSplit`. Un nodo con el fondo bajo `waterLevelM` se parte, aunque su error en
 * pantalla no lo pida, mientras su téxel esconda más de `waterReliefFrac` × profundidad de relieve.
 * `waterReach` acota el coste: sólo se aplica a nodos cuyo error en pantalla pasa de
 * `errorPx / waterReach`, o sea a los que están a menos de `waterReach` veces su distancia de
 * partición normal. Sin ese tope la regla se aplicaría a toda plataforma continental visible desde
 * órbita (un banco de < 15 m de 100 000 km² pediría ~17 000 nodos de nivel 12) y saturaría el
 * presupuesto cada frame. Con él, el coste extra es un múltiplo acotado de lo que ya se dibuja
 * alrededor de la cámara (medido: +80 % con alcance 2 en el peor caso, ver
 * `TERRAIN_NODE_WATER_SPLIT_REACH`) — y los círculos que quedan se van a `waterReach` veces más
 * lejos, donde el nodo mide `waterReach` veces menos en pantalla. `outWaterSplit` cuenta cuántas
 * particiones las pidió el agua y no la pantalla.
 */
/**
...
inline void nodeSelectVisible(double planetRadiusM, const glm::dvec3& camPos,
                              ...
                              double waterLevelM = -1e30, double waterReliefFrac = 0.0,
                              double waterReach = TERRAIN_NODE_WATER_SPLIT_REACH,
                              size_t* outWaterSplit = nullptr) {
    out.clear();
    const uint32_t maxLevel = nodeMaxLevelFor(planetRadiusM);
    const bool waterRule = (waterReliefFrac > 0.0) && rangeFn;
    ...
    if (outWaterSplit)    *outWaterSplit    = 0;
    ...
    while (!stack.empty()) {
        ...
        bool wants = (n.level < maxLevel && cand.err > errorPx);
        // EL AGUA SOMERA PIDE MAS QUE LA PANTALLA (ver `nodeShallowWantsSplit` y la nota de arriba
        // sobre `waterReach`). Va detras del criterio de pantalla: solo se consulta el rango si la
        // pantalla ya dijo que no, y solo dentro del alcance.
        if (!wants && waterRule && n.level < maxLevel && cand.err * waterReach > errorPx) {
            const NodeRange r = rangeFn(n, rangeUser);
            if (r.valid() && nodeShallowWantsSplit(n, planetRadiusM, r.minM, waterLevelM, waterReliefFrac)) {
                wants = true;
                if (outWaterSplit) ++*outWaterSplit;
            }
        }
