...
struct TerrainMaterial {
    ...
    /// CUÁNTA HIERBA DENSA crece sobre este material, 0..1 (ver `renderer/grass_renderer.h`). En el
    /// JSON es `"grass"`; sin declarar (−1) vale 1 si el albedo se llama `grass*` y 0 si no: la misma
    /// regla que ya pinta el suelo de verde pone la hierba, sin una segunda tabla que la contradiga.
    float     grass  = -1.0f;
    float grassAmount() const {
        if (grass >= 0.0f) return grass;
        return (albedo.find("grass") != std::string::npos) ? 1.0f : 0.0f;
    }
