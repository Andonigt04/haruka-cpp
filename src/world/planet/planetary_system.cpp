...
#include "world/terrain/terrain_grid.h"
#include "world/terrain/terrain_lod.h"   // terrainTriM: el piso del campo cercano, no un literal
#include "world/water/water_fill.h"   // WATER_FILL_DRY: el centinela del mapa de lagos
#include "world/planet/geology.h"       // GeologyConfig (addSimplePlanet/rebuildSimplePlanet)
#include "world/water/ocean_wave.h"   // gemelo CPU del oleaje: la física ve la MISMA ola
#include "renderer/primitive_shapes.h"
#include "core/components/mesh_renderer_component.h"
#include "world/props/prop_layer.h"           // PropLayerTable (propLayers del surfaceConfig)
    ...
    if (auto* tp = activeTerrestrialMut()) {
        ...
        if (getActivePlanet(pc, pr) && pr > 0.0) {
            ...
            if (len > 1e-9) {
                ...
                static int s_log = 0;
                if ((s_log++ % 20) == 0)
                    HARUKA_LOGDIAG("Vox", "trazo %s r=%.1f m: %d voxeles cambian", dig ? "PICAR" : "CONSTRUIR", radius, n);
                return;
            }
        }
    }
...
void PlanetarySystem::prepareSimplePlanets(const glm::dvec3& cameraPos, const glm::dvec3& viewDir,
    ...
    for (auto& p : m_simplePlanets) p->prepare(cameraPos, viewDir, fovYRad, aspect, viewportH, viewUp);
}
