...
#include "world/planet/planet_fields.h"        // FieldSample (fieldSampleAt)
#include "world/water/ocean_wave.h"            // OceanState: el estado del mar que se sube a la GPU
#include "world/terrain/terrain_node_renderer.h" // v5: pase de terreno por nodos (HARUKA_TERRAIN_V5=1)
#include "world/vox/vox_world.h"             // el campo volumétrico: cuevas y lo que se pica
#include "renderer/vox_renderer.h"
#include "renderer/grass_renderer.h"     // hierba densa en GPU sobre el pase de nodos
#include "world/weather_system.h"
#include "world/water/water_fill.h"          // WaterWindowResult: el relleno FINO de una ventana
#include <future>
#include <mutex>
...
public:
    ...
    /// `viewUp` = el arriba de la CAMARA (para los planos del frustum de la hierba; con (0,0,0)
    /// se usa el radial del planeta, que solo falla si la camara esta muy inclinada).
...
private:
    ...
    /// LA HIERBA DENSA (primer plano, ~60 m): generada en GPU sobre los nodos que dibuja el pase v5
    /// y aplastada por lo que se mueve (ver `renderer/grass_renderer.h`). Va pegada al pase de nodos
    /// porque lee SU mapa de alturas: sin v5 no hay hierba densa (queda la capa de props).
    Haruka::Renderer::GrassRenderer m_grass;
    std::vector<Haruka::Terrain::TerrainNodeRenderer::NearNode> m_grassNodes;
    glm::vec3 m_grassWind{0.0f};
    float     m_grassTime = 0.0f;
...
public:
    ...
    /// La hierba: quien se mueva por el suelo la aplasta con `grass().addStamp(...)` (jugador,
    /// criaturas, objetos con fisica). El viento y el reloj los pone quien los tenga (la app).
    Haruka::Renderer::GrassRenderer& grass() { return m_grass; }
    void setGrassWind(const glm::vec3& windWorld, float timeS) { m_grassWind = windWorld; m_grassTime = timeS; }
